...
import type { KindWizardFormState } from "../wizard-types";
...
export function StepUebersichtKind({ value, kostentraegerOptions }: Props) {
  const kt = kostentraegerOptions.find((o) => o.id === value.kostentraegerId);
  return (
      ...
      <Section title="Stammdaten">
        ...
        <Row
          ...
          value={
            value.schule.name
              ? `${value.schule.name}${
                  value.schule.address ? ` — ${value.schule.address}` : ""
                }`
              : "—"
          }
        />
