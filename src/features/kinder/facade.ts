...
function childFieldsFromCreate(input: CreateKindInput) {
  return {
    ...
    schoolName: input.schule?.name ?? null,
    schoolAddress: input.schule?.address ?? null,
    schoolPlaceId: input.schule?.placeId ?? null,
    schoolLat: input.schule?.lat ?? null,
    schoolLng: input.schule?.lng ?? null,
  };
}

function childFieldsFromUpdate(input: UpdateKindInput) {
  ...
  if (input.schule !== undefined) {
    out.schoolName = input.schule?.name ?? null;
    out.schoolAddress = input.schule?.address ?? null;
    out.schoolPlaceId = input.schule?.placeId ?? null;
    out.schoolLat = input.schule?.lat ?? null;
    out.schoolLng = input.schule?.lng ?? null;
  }
  return out;
}

export const KinderFacade = {
  async list() {
    return listChildren();
  },
  ...
  async create(input: CreateKindInput) {
    const parsed = CreateKindSchema.parse(input);
    return createChild(childFieldsFromCreate(parsed));
  },

  async update(id: string, input: UpdateKindInput) {
