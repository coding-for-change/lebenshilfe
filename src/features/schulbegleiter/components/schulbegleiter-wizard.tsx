...
import {
  createSchulbegleiterAction,
  updateSchulbegleiterAction,
} from "../actions";
import {
...
import { StepStammdaten } from "./steps/step-stammdaten";
import { StepVertrag } from "./steps/step-vertrag";
import { StepWorkshops } from "./steps/step-workshops";
import { StepUebersicht } from "./steps/step-uebersicht";
import {
  STEP_LABELS,
  type WizardErrors,
...
function buildSubmitInput(value: WizardFormState): CreateSchulbegleiterInput {
  const weeklyHours =
...
export function SchulbegleiterWizard({
  open,
  onOpenChange,
  workshops,
  edit,
  ...
  async function handleSubmit() {
    ...
    try {
      if (mode === "edit" && edit) {
        await updateSchulbegleiterAction(edit.id, submitInput);
        toast.success("Schulbegleiter aktualisiert.");
      } else {
        await createSchulbegleiterAction(submitInput);
